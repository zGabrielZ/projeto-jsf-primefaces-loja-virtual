...
@Getter
@Setter
public class UsuarioLoteExcelController extends AbstractControllerLote{
	...
	public void uploadArquivo() {
		if (arquivoUploadDados.getContentType()
				.equals("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") && usuarios.isEmpty()) {
			...
			try {
				...
				usuarioLote = usuarioService.verificarArquivoExcel(arquivoUploadDados);
	...
	public StreamedContent getModeloArquivoDados() {
		...
		try {
			...
			modeloArquivoDados = new DefaultStreamedContent(inputStream, "application/xlsx","modelo-usuario.xlsx");
		} catch (Exception e) {
	...
	private Usuario getUsuarioLoteToUsuario(UsuarioLoteTo usuarioLote){
		...
		usuario.setCodigoUsuario(usuarioLote.getCodigoUsuario());
